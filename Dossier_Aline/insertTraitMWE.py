...
def traitementConll(fichierEntree,dicoMWE,fichierSortie="resultatTraitement.conllu"):
    """
            ...
            for x in range(0,len(phrase)):
                if phrase[x:x+l]==mwe2[0:l]: #on cherche la mwe dans la liste de lemme (ou de tokens)
                    ...
                    #on va remplacer la 10e colonne, mais deux cas : tête ou dépendant
                        if int(dicoLignes[str(y)][2]) in range(x+1,x+l+1): #possède un gouverneur parmi la mwe
                            modif=re.sub(dicoLignes[str(y)][1]+'$','INMWE=yes',dicoLignes[str(y)][0])
                        else: #n'en possède pas
                            modif=re.sub(dicoLignes[str(y)][1]+'$','MWEPOS='+dicoMWE[mwe],dicoLignes[str(y)][0])                                          
